...
struct EstimatedTimeView: View {
    ...
    var body: some View {
        let progress = totalCharCount > 0 ? Double(highlightedCharCount) / Double(totalCharCount) : 0
        let wordsRead = Int(Double(totalWords) * progress)
        let wordsRemaining = max(0, totalWords - wordsRead)
        // Use configured scroll speed as approximate WPS, minimum 1.0
        let wps = max(1.0, NotchSettings.shared.scrollSpeed)
        let secondsRemaining = Int(Double(wordsRemaining) / wps)
...
struct WPMIndicatorView: View {
    ...
    var body: some View {
        if let start = startTime {
            let wpm = TimerEstimator.wordsPerMinute(startTime: start, wordsCompleted: wordsCompleted)
            if wpm > 0 {
                HStack(spacing: 2) {
                    Text("\(wpm)")
